    ...
    <div class="w-full h-screen flex flex-row justify-center pt-16 ">
     <aside id="default-sidebar" class="fixed left-0 mt-14 h-full z-40 w-64" aria-label="Sidenav">
      ...
      <ul class="pt-5 mt-5 space-y-2 border-t border-gray-200 dark:border-gray-700">
          ...
          <li>
              <a href="/course-pilot/instructor/announcement" class="flex text-white items-center bg-green-600 hover:bg-green-800 p-2 text-base font-normal text-gray-900 rounded-lg transition duration-75 hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-white group">
                  <svg aria-hidden="true" class="flex-shrink-0 hover:text-white w-6 h-6 text-white transition duration-75" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-2 0c0 .993-.241 1.929-.668 2.754l-1.524-1.525a3.997 3.997 0 00.078-2.183l1.562-1.562C15.802 8.249 16 9.1 16 10zm-5.165 3.913l1.58 1.58A5.98 5.98 0 0110 16a5.976 5.976 0 01-2.516-.552l1.562-1.562a4.006 4.006 0 001.789.027zm-4.677-2.796a4.002 4.002 0 01-.041-2.08l-.08.08-1.53-1.533A5.98 5.98 0 004 10c0 .954.223 1.856.619 2.657l1.54-1.54zm1.088-6.45A5.974 5.974 0 0110 4c.954 0 1.856.223 2.657.619l-1.54 1.54a4.002 4.002 0 00-2.346.033L7.246 4.668zM12 10a2 2 0 11-4 0 2 2 0 014 0z" clip-rule="evenodd"></path></svg>
...
<main class="" style="width: 1200px; margin-left: 240px; margin-top: 30px;">
    <div class="bg-white w-50 dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <header class="text-center">
            ...
            <p class="text-lg text-gray-600 dark:text-gray-400">
                Share your insights or add to the discussion!
            </p>
        </header>

        <section class="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-lg">
            <h2 class="text-3xl font-semibold text-gray-900 dark:text-white mb-6 text-center">
                New Post
            </h2>
            <form class="space-y-6" method="POST" action="/course-pilot/instructor/create-post">
                <div>
                    <label for="discussion" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">
                        Select Discussion
                    </label>
                    <select
                        id="discussion"
                        name="discussion_id"
                        class="w-full p-4 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                        required>
                        <option value="" disabled selected>Select a discussion</option>
                        <?php if (!empty($discussions)): ?>
                            <?php foreach ($discussions as $discussion): ?>
                                <option value="<?= htmlspecialchars($discussion['discussion_id']) ?>">
                                    <?= htmlspecialchars($discussion['discussion_title']) ?>
                                </option>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <option value="" disabled>No discussions available</option>
                        <?php endif; ?>
                    </select>
                </div>

                <div>
                    <label for="post-content" class="block text-gray-700 dark:text-gray-300 font-medium mb-2">
                        Post Content
                    </label>
                    <textarea
                        id="post-content"
                        name="content"
                        rows="6"
                        class="w-full p-4 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                ...
                <button
                    type="submit"
                    class="w-full bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white py-3 rounded-lg font-semibold focus:outline-none focus:ring-4 focus:ring-blue-300">
                    Submit Post
                </button>
            </form>
        </section>
