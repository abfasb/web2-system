...
$page = isset($_GET['panel']) ? $_GET['panel'] : 'dashboard';

//dashboard, add new course, manage resources, assignments and grading, quiz and test centre, view student progress, 
//track grades and completion rates, classroom discussion, reports and analytics, discussion forums, resource library
?>
     ...
     <aside id="default-sidebar" class="fixed top-30 mt-14 h-full z-40 w-64" aria-label="Sidenav">
      <div class="overflow-y-auto py-5 px-3 h-full bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        <ul class="space-y-2">
          ...
          <li>
              <button type="button" class="flex bg-green-600 hover:bg-green-800 text-white items-center p-2 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" aria-controls="dropdown-authentication" data-collapse-toggle="dropdown-authentication">
                  <svg aria-hidden="true" class="flex-shrink-0 text-white hover:text-white w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"></path></svg>
...
<main class="p-8 ml-[300px] w-[1200px] pl-14 flex justify-center items-center">
    <div class="bg-white w-50 dark:bg-gray-800 p-6 rounded-lg shadow-md" style="width:1280px; margin-left:230px;">
    <div class="flex items-center justify-between mb-12">
            <h4 class="text-4xl font-bold text-gray-800 dark:text-gray-200 mb-4">Student Tracking</h4>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php foreach ($courses as $course): ?>
                <a href="/course-pilot/instructor/student-progress?course_id=<?= $course['course_id'] ?>" 
                class="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 p-6 mt-4 rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transform transition-all duration-300 ease-in-out flex flex-col">
                    <div class="mb-4 flex-grow">
                        <h3 class="text-2xl font-semibold text-gray-800 mb-2"><?= htmlspecialchars($course['title']) ?></h3>
                        
                        <p class="text-sm text-gray-600">Added on: <span class="font-medium text-gray-800"><?= date('M d, Y', strtotime($course['created_at'])) ?></span></p>
                        <p class="text-sm text-gray-600">Code: <span class="font-medium text-gray-800"><?= htmlspecialchars($course['course_code']) ?></span></p>
                    </div>
                    ...
                    <div class="mt-4 text-right">
                        <span class="bg-indigo-500 text-white px-4 py-2 rounded-lg text-xs font-semibold tracking-wider uppercase hover:bg-indigo-600 cursor-pointer transition-all duration-300 ease-in-out">
                            View Progress
                        </span>
                    </div>
                </a>
