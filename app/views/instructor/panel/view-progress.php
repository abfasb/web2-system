...
<main class="p-6 max-w-7xl mx-auto flex items-center justify-center" style="margin-left:240px;">
    <section class="bg-white shadow-lg rounded-lg p-8" style="width:1200px;">
        <div class="border-b border-gray-300 pb-4 mb-6 flex justify-between items-center">
            <button 
                onclick="window.history.back()" 
                class="px-4 py-2 text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 transition-all"
            >
                Return to Back
            </button>
            <h2 class="text-3xl font-bold text-gray-800">Course: <?= htmlspecialchars($course['title']) ?></h2>
        </div>
        ...
        <div class="overflow-x-auto bg-gray-50 rounded-lg shadow-md">
            <table class="w-full bg-white border-collapse table-auto">
                <thead class="bg-gradient-to-r from-blue-500 to-blue-600 text-white">
                    <tr>
                        <th class="py-4 px-6 text-left font-medium text-lg">Student Name</th>
                        <th class="py-4 px-6 text-left font-medium text-lg">Assignments Completed</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($students as $student): ?>
                        <tr class="border-b border-gray-200 hover:bg-blue-50 transition-all">
                            <td class="py-4 px-6 text-left text-gray-800"><?= htmlspecialchars($student['username']) ?></td>
                            <td class="py-4 px-6">
                                <div class="flex items-center">
                                    <div class="w-full bg-gray-200 rounded-full h-5 overflow-hidden">
                                        <div 
                                            class="bg-gradient-to-r from-green-400 to-green-600 h-5 rounded-full" 
                                            style="width: <?= $student['assignments'] ?>%; transition: width 0.4s ease;"
                                        ></div>
                                    </div>
